...
class Category(models.Model, ImageProcessingMixin):
    ...
    parent = models.ForeignKey('self', verbose_name=_('Родительская категория'), on_delete=models.CASCADE,
...
class Product(models.Model, ImageProcessingMixin):
    ...
    supplier_integration = models.ForeignKey(Supplier, verbose_name=_('Поставщик'), on_delete=models.CASCADE, null=True,
                                             blank=True)
    ...
    discount_price = models.DecimalField(verbose_name=_('Цена со скидкой'), max_digits=12, decimal_places=6, null=True,
                                         blank=True)
    discount = models.DecimalField(verbose_name=_('Скидка'), max_digits=12, decimal_places=6, default=0)
    discount_type = models.CharField(verbose_name=_('Тип скидки'), max_length=255, null=True, blank=True,
...
class PromotionalProduct(models.Model):
    ...
    required_quantity = models.PositiveIntegerField(default=1,
                                                    verbose_name='Количество для подарка')  # Количество товаров для получения подарка
